...
*/
  ...
  // Split gaussian into large and narrow ones to avoid Eta too large
  // not the best idea -> lower prefac instead
  /*
  ...
  */
  ...
  virtual void initializeGaussian(double alpha, double a_latt, int ncells, double prefac)
  {
    double atoms[4][3];
        ...
        for(int k=0;k<Nz_; k++)
          {
            ...
            // TODO: was 1e-4 before without causing problems
            //       but 1e-4 is of the same order of magnitude that of
...
// Take into account contributions from gaussian in cells as far as 
// ncells_range

virtual void initializeGaussian2(double alpha, double a_latt, int ncells_range ,double prefac)
  {
    double atoms[4][3];
        ...
        for(int k=0;k<Nz_; k++)
          {
            ...
            for(int icell=-ncells_range;icell < ncells_range; icell++)
              for(int jcell=-ncells_range;jcell < ncells_range; jcell++)
                for(int kcell=-ncells_range;kcell < ncells_range; kcell++)
                  for(int l=0;l<4;l++)         
                    {
                      double dx = fabs(x-atoms[l][0]-icell*a_latt);
                      double dy = fabs(y-atoms[l][1]-jcell*a_latt);
                      double dz = fabs(z-atoms[l][2]-kcell*a_latt);
...
virtual void initializeUniform(double rho)
  {
    for(int i=0;i<Nx_;i++)
      for(int j=0;j<Ny_;j++)
        for(int k=0;k<Nz_; k++)
