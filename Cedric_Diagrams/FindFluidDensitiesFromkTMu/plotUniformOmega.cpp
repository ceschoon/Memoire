...
string to_string_no_zeros(double num)
{
...
// Program to plot the Omega(rho) curve for the uniform phase
...
int main(int argc, char** argv)
{
	...
	*/
	...
	outPlotFile << "########## Plot Uniform Free Energy #########" << endl;
	outPlotFile << endl;
	outPlotFile << "set term epslatex color" << endl;
	outPlotFile << "set output 'Omega.tex'" << endl;
	outPlotFile << endl;
	outPlotFile << "set title \"Uniform Free Energy\" font \",20\"" << endl;
	outPlotFile << "set xlabel \"$\\\\rho \\\\sigma^3$\" font \",20\"" << endl;
	outPlotFile << "set ylabel \"$\\\\omega / \\\\epsilon$\" font \",20\"" << endl;
	outPlotFile << "set style data lines" << endl;
	outPlotFile << "set grid" << endl;
	outPlotFile << "set key top left" << endl;
	if (yRangeMin!=0 && yRangeMin!=0) 
	...
	for (int j=0; j<sizeMu; j++)
	{
		...
		for (int i=0; i<sizeRho; i++) 
		{
			...
			Omega[i] = uniformOmega(kT,mu[j],aVdW,hsd1,rho[i]);
		}
		...
		*/
		...
		if (sizeMu==1)
			outPlotFile << "plot 'Omega_"+to_string(j)+".dat' title '$k_BT="+to_string_no_zeros(kT)+",\\mu="+to_string_no_zeros(mu[j])+"$'" << endl;
		else if (j==0)
			outPlotFile << "plot 'Omega_"+to_string(j)+".dat' title '$k_BT="+to_string_no_zeros(kT)+",\\mu="+to_string_no_zeros(mu[j])+"$', \\" << endl;
		else if (j==sizeMu-1)
			outPlotFile << "     'Omega_"+to_string(j)+".dat' title '$k_BT="+to_string_no_zeros(kT)+",\\mu="+to_string_no_zeros(mu[j])+"$'" << endl;
		else 
			outPlotFile << "     'Omega_"+to_string(j)+".dat' title '$k_BT="+to_string_no_zeros(kT)+",\\mu="+to_string_no_zeros(mu[j])+"$', \\" << endl;
