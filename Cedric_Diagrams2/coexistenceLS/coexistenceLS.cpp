...
int muComputation( double kT, double mu, 
                   ...
                   vector<double> &freeEnergyLiquid_vec,
                   vector<double> &densityLiquid_vec)
{
	...
	// Compute free energy for the liquid
	
	double freeEnergyVapour, densityVapour;
	double freeEnergyLiquid, densityLiquid;
	bool supercritical;
	
	int statusLiquid = fixedkTMuFluid2(kT, mu, argc, argv, log, 
		freeEnergyVapour, densityVapour, freeEnergyLiquid, densityLiquid,
		supercritical);
	
	// Save results
	
	freeEnergyDiff = freeEnergyLiquid-freeEnergySolid;
	
	if (statusSolid==0 && statusLiquid==0)
	{
		mu_vec.push_back(mu);
		...
		freeEnergyLiquid_vec.push_back(freeEnergyLiquid);
		densityLiquid_vec.push_back(densityLiquid);
	}
	...
	else
	{
		...
		if (statusLiquid!=0)
			log << "ERROR: Liquid Computation FAILED" << endl;
	}
	
	// return status
	
	if (statusSolid==0 && statusLiquid==0) return 0;
	return 1;
}
...
int main(int argc, char** argv)
{
	        ...
	        (kTStep<0 && kT>=kTStop) )  // stop condition if kTStep<0
	{
		...
		ifstream dataInFile(dataDir+"/coexistenceLS_"+"kT="+sskT.str()+".dat");
		...
		double freeEnergyDiff_0; // liquid - solid
		...
		int status_0 = muComputation(kT, mu_0, argc, argv, log, 
			...
			freeEnergyLiquid_vec, densityLiquid_vec);
		...
		for (int i=0; i<mu_vec.size(); i++)
		{
			...
			temp = freeEnergyLiquid_vec[i]; 
			freeEnergyLiquid_vec[i]     = freeEnergyLiquid_vec[j_min]; 
			freeEnergyLiquid_vec[j_min] = temp;
			
			temp = densityLiquid_vec[i]; 
			densityLiquid_vec[i]     = densityLiquid_vec[j_min]; 
			densityLiquid_vec[j_min] = temp;
		}
		...
		log << endl;
		log << "freeEnergyLiquid_vec = ";
		for (int i=0; i<freeEnergyLiquid_vec.size(); i++)
			log << freeEnergyLiquid_vec[i] << " ";
		
		log << endl;
		log << "densityLiquid_vec = ";
		for (int i=0; i<densityLiquid_vec.size(); i++)
			log << densityLiquid_vec[i] << " ";
		
		// find the coexistence value for mu by searching the zero of
		// freeEnergyLiquid - freeEnergySolid (using interpolation)
		
		vector<double> freeEnergyDiff_vec(mu_vec.size()); 
		for (int i=0; i<mu_vec.size(); i++)
			freeEnergyDiff_vec[i] = freeEnergyLiquid_vec[i] - freeEnergySolid_vec[i];
		...
		if (freeEnergyDiff_vec[0] * freeEnergyDiff_vec[freeEnergyDiff_vec.size()-1] < 0)
		{
			...
			evalFromDataInterpolation(mu_vec, densityLiquid_vec, muCoex, densityLiquidCoex);
		}
		else
		{
			log <<  myColor::RED << "=================================" << myColor::RESET << endl << "#" << endl;
		...
		ofstream dataFile(dataDir+"/coexistenceLS_"+"kT="+sskT.str()+".dat");
		
		dataFile << "# Result for liquid-solid coexistence at " << endl;
		...
		dataFile << "densityLiquidCoex = " << scientific << setprecision(8) << densityLiquidCoex << endl;
		dataFile << endl;
