...
int main(int argc, char** argv)
{
	...
	ofstream outDataFile("coexCurveLS.dat");
	...
	for (double kT=kTMin; kT<=kTMax; kT+=kTStep)
	{
		...
		if (dataInFile)
		{
			...
			if (success==1)  // bool "true" 
			{
				...
				double rhoL, rhoS, muCoex, freeEnergyCoex, Ngrid, Cvac, alpha;
				readDataFromFile(dataInFile, "muCoex", muCoex);
				readDataFromFile(dataInFile, "freeEnergyCoex", freeEnergyCoex);
				readDataFromFile(dataInFile, "densityLiquidCoex", rhoL);
	...
	outPlotFile << "set output 'coexCurveLS.svg'" << endl;
	outPlotFile << endl;
	outPlotFile << "set title \"Coexistence Liquid-Solid\" font \",20\"" << endl;
	...
	outPlotFile << endl;
	outPlotFile << "plot 'coexCurveLS.dat' using 2:1 title 'liquid', \\" << endl;
	outPlotFile << "     'coexCurveLS.dat' using 3:1 title 'solid' " << endl;
