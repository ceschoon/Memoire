                   ...
                   string fluidType)
{
	...
	// Select the right fluid type 
	
	double freeEnergyFluid, densityFluid;
	
	if (fluidType == "fluid")
	{
		if (freeEnergyVapour < freeEnergyLiquid)
		{
			freeEnergyFluid = freeEnergyVapour;
			densityFluid = densityVapour;
		}
		else
		{
			freeEnergyFluid = freeEnergyLiquid;
			densityFluid = densityLiquid;
		}
	}
...
int main(int argc, char** argv)
{
	        ...
	        (kTStep<0 && kT>=kTStop) )  // stop condition if kTStep<0
	{
		...
		int status_0 = muComputation(kT, mu_0, argc, argv, log, 
			...
			freeEnergyFluid_vec, densityFluid_vec, fluidType);
		...
		dataFile << endl;
		dataFile << "fluidType = " << fluidType << endl;
		dataFile << endl;
