...
// Program to plot the d2Omega/drho2 curve for the uniform liquid
...
int main(int argc, char** argv)
{
	...
	outPlotFile << "########## Plot Uniform Free Energy Second Derivative #########" << endl;
	outPlotFile << endl;
	outPlotFile << "set term svg enhanced mouse #size 800,600" << endl;
	outPlotFile << "set output 'd2Omegadrho2.svg'" << endl;
	outPlotFile << endl;
	outPlotFile << "set title \"Uniform Free Energy Second Derivative\" font \",20\"" << endl;
	outPlotFile << "set xlabel \"rho\" font \",20\"" << endl;
	outPlotFile << "set ylabel \"d2Omega/drho2\" font \",20\"" << endl;
	...
	for (int j=0; j<sizeMu; j++)
	{
		...
		vector<double> d2Omegadrho2(sizeRho);
		
		for (int i=0; i<sizeRho; i++) 
		{
			rho[i] = rhoMin + rhoStep*i;
			d2Omegadrho2[i] = uniformFluidOmegaDerivative2(kT,mu[j],aVdW,hsd1,rho[i]);
		}
		
		// Save data to plot
		
		ofstream outDataFile("d2Omegadrho2_"+to_string(j)+".dat");
		
		for (int i=0; i<sizeRho; i++)
			outDataFile << rho[i] << " " << d2Omegadrho2[i] << endl;
		
		if (sizeMu==1)
			outPlotFile << "plot 'd2Omegadrho2_"+to_string(j)+".dat' title 'kT="+to_string(kT)+",mu="+to_string(mu[j])+"'" << endl;
		else if (j==0)
			outPlotFile << "plot 'd2Omegadrho2_"+to_string(j)+".dat' title 'kT="+to_string(kT)+",mu="+to_string(mu[j])+"', \\" << endl;
		else if (j==sizeMu-1)
			outPlotFile << "     'd2Omegadrho2_"+to_string(j)+".dat' title 'kT="+to_string(kT)+",mu="+to_string(mu[j])+"'" << endl;
		else 
			outPlotFile << "     'd2Omegadrho2_"+to_string(j)+".dat' title 'kT="+to_string(kT)+",mu="+to_string(mu[j])+"', \\" << endl;
