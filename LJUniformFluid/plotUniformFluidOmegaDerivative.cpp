...
// Program to plot the dOmega/drho curve for the uniform liquid
...
int main(int argc, char** argv)
{
	...
	outPlotFile << "########## Plot Uniform Free Energy Derivative #########" << endl;
	outPlotFile << endl;
	outPlotFile << "set term svg enhanced mouse #size 800,600" << endl;
	outPlotFile << "set output 'dOmegadrho.svg'" << endl;
	outPlotFile << endl;
	outPlotFile << "set title \"Uniform Free Energy Derivative\" font \",20\"" << endl;
	outPlotFile << "set xlabel \"rho\" font \",20\"" << endl;
	outPlotFile << "set ylabel \"dOmega/drho\" font \",20\"" << endl;
	...
	for (int j=0; j<sizeMu; j++)
	{
		...
		vector<double> dOmegadrho(sizeRho);
		
		for (int i=0; i<sizeRho; i++) 
		{
			rho[i] = rhoMin + rhoStep*i;
			dOmegadrho[i] = uniformFluidOmegaDerivative(kT,mu[j],aVdW,hsd1,rho[i]);
		}
		
		// Save data to plot
		
		ofstream outDataFile("dOmegadrho_"+to_string(j)+".dat");
		
		for (int i=0; i<sizeRho; i++)
			outDataFile << rho[i] << " " << dOmegadrho[i] << endl;
		
		if (sizeMu==1)
			outPlotFile << "plot 'dOmegadrho_"+to_string(j)+".dat' title 'kT="+to_string(kT)+",mu="+to_string(mu[j])+"'" << endl;
		else if (j==0)
			outPlotFile << "plot 'dOmegadrho_"+to_string(j)+".dat' title 'kT="+to_string(kT)+",mu="+to_string(mu[j])+"', \\" << endl;
		else if (j==sizeMu-1)
			outPlotFile << "     'dOmegadrho_"+to_string(j)+".dat' title 'kT="+to_string(kT)+",mu="+to_string(mu[j])+"'" << endl;
		else 
			outPlotFile << "     'dOmegadrho_"+to_string(j)+".dat' title 'kT="+to_string(kT)+",mu="+to_string(mu[j])+"', \\" << endl;
